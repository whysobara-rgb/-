...
adb('install', '-r', 'build/deliverables/v33-ui-probe-debug.apk')
adb('logcat', '-c')
adb('shell', 'am', 'start', '-W', '-n', package + '/com.gachavault.gacha.MainActivity')
pending = {'HOME', 'SHOP', 'DETAIL'}
try:
    deadline = time.monotonic() + 90
    while pending and time.monotonic() < deadline:
        logs = adb('logcat', '-d', '-s', 'flutter:I', '*:S')
        if 'V33_UI_PROBE_FAILED' in logs:
            raise RuntimeError('Flutter UI error; see v33-ui.log')
        for screen in tuple(pending):
            if 'V33_' + screen + '_READY' in logs:
                with (out / ('v33-' + screen.lower() + '.png')).open('wb') as stream:
    ...
    (out / 'v33-result.json').write_text(json.dumps({
    ...
    (out / 'v33-ui.log').write_text(adb('logcat', '-d', '-s', 'flutter:I', '*:S', check=False))
    adb('shell', 'am', 'force-stop', package)
