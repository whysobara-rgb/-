...
adb('install', '-r', 'build/deliverables/v33-stage3-probe-debug.apk')
adb('logcat', '-c')
adb('shell', 'am', 'start', '-W', '-n', package + '/com.gachavault.gacha.MainActivity')
pending = {'ORDERS', 'REFUND', 'SHIPPING', 'SECURITY', 'CLOSURE', 'CONVERSION'}
try:
    deadline = time.monotonic() + 90
    while pending and time.monotonic() < deadline:
        logs = adb('logcat', '-d', '-s', 'flutter:I', '*:S')
        if 'V33_STAGE3_FAILED' in logs:
            raise RuntimeError('Flutter UI error; see v33-stage3-ui.log')
        for screen in tuple(pending):
            if 'V33_STAGE3_' + screen + '_READY' in logs:
                with (out / ('v33-stage3-' + screen.lower() + '.png')).open('wb') as stream:
    ...
    (out / 'v33-stage3-result.json').write_text(json.dumps({
        'result': 'PASS', 'screens': ['Orders', 'Refund', 'Shipping', 'Security', 'Closure', 'Conversion'],
        'source_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
        'device': 'Android API 35 x86_64 emulator', 'data': 'synthetic UI fixtures',
        'live_transactions': False,
    }, indent=2))
finally:
    (out / 'v33-stage3-ui.log').write_text(adb('logcat', '-d', '-s', 'flutter:I', '*:S', check=False))
    adb('shell', 'am', 'force-stop', package)
